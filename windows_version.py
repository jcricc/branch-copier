...
class ProjectCopier(QWidget):
    def __init__(self):
        ...
        help_label.setText(
            "<b>\u2328 Shortcuts:</b><br>"
            f"<b>{'Ctrl' if platform.system() == 'Windows' else 'Cmd'} + O</b> → Open folder<br>"
            f"<b>{'Ctrl' if platform.system() == 'Windows' else 'Cmd'} + F</b> → Focus search<br>"
            f"<b>{'Ctrl' if platform.system() == 'Windows' else 'Cmd'} + Shift + C</b> → Copy file content<br>"
            f"<b>{'Ctrl' if platform.system() == 'Windows' else 'Cmd'} + Shift + S</b> → Copy structure<br>"
        ...
        # Keyboard shortcuts
        prefix = "Ctrl" if platform.system() == "Windows" else "Meta"
        QShortcut(QKeySequence(f"{prefix}+O"), self, self.choose_folder)
        QShortcut(QKeySequence(f"{prefix}+F"), self, self.focus_search)
        QShortcut(QKeySequence(f"{prefix}+Shift+C"), self, self.copy_preview_contents)
        QShortcut(QKeySequence(f"{prefix}+Shift+S"), self, self.copy_folder_structure)
